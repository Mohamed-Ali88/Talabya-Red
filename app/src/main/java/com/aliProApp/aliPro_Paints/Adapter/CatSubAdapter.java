...
public class CatSubAdapter extends RecyclerView.Adapter<CatSubAdapter.ViewHolder> {

    private ArrayList<comCatDomain> catDomain;

    public CatSubAdapter(ArrayList<comCatDomain> CatDomain) {
        this.catDomain = CatDomain;
    }

    @NonNull
    @Override
    public CatSubAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View inflate = LayoutInflater.from(parent.getContext()).inflate(R.layout.viewholder_cat_and_companies, parent, false);
        return new ViewHolder(inflate);
    }

    @Override
    public void onBindViewHolder(@NonNull CatSubAdapter.ViewHolder holder, int position) {
        ...
        holder.catLy.setOnClickListener(view -> {
            ...
            intent.putExtra("object2", catDomain.get(position));
            intent.putExtra("comOrCatProducts", "category");
