...
public class TinyDB {
    ...
    public ArrayList<OrderDetailsData> getListObject2(String key){
        Gson gson = new Gson();

        ArrayList<String> objStrings = getListString(key);
        ArrayList<OrderDetailsData> playerList =  new ArrayList<OrderDetailsData>();

        for(String jObjString : objStrings){
            OrderDetailsData player  = gson.fromJson(jObjString,  OrderDetailsData.class);
            playerList.add(player);
        }
        return playerList;
    }
    ...
    public void putListObject2(String key, ArrayList<OrderDetailsData> playerList){
        checkForNullKey(key);
        Gson gson = new Gson();
        ArrayList<String> objStrings = new ArrayList<String>();
        for(OrderDetailsData player: playerList){
            objStrings.add(gson.toJson(player));
        }
        putListString(key, objStrings);
    }
